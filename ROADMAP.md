...
## Этап 8 — UX и автоматизация ✅

### 8.1. Мини-дашборд

**Зачем:** быстро оценить нагрузку — всего/новых/в работе/завершённых + распределение по приоритетам.

- [x] `GET /tickets/stats` — один эндпоинт, агрегаты через SQLAlchemy `group_by`
- [x] `DashboardStats.tsx` — 4 карточки над таблицей, горизонтальная полоса приоритетов
- [x] TanStack Query — инвалидация при создании/изменении/удалении заявки
- [x] 0 новых зависимостей

### 8.2. Баннер «зависших» заявок

**Зачем:** не дать заявкам висеть в `in_progress` неделями.
...
**Зачем:** пользователь сохраняет частые комбинации фильтров и переключается в один клик.

- [x] `SavedFiltersDropdown.tsx` — выпадающий список пресетов + кнопка «Сохранить»
- [x] `useSavedFilters.ts` — хук: чтение/запись/удаление пресетов в localStorage
- [x] Каждый пресет: имя + `TicketFilters` (search, status, priority, sortBy, order)
- [x] Только фронт — 0 изменений на бэкенде

### Итого по этапу

3 компонента, 1 эндпоинт, 0 новых зависимостей.  
Каждая фича закрывает слово из вакансии: **дашборд** = удобство, **сохранённые фильтры** = автоматизация рутины, **баннер зависших** = контроль.